...
Email.prototype.attachEvents = function attachEvents(app) {
  ...
  $('#reply').on('click', function() {
    ...
    if (email_self.app.keys.hasSharedSecret(fr)) {
      ...
      //if (email_self.active_module == "Email") {
...
Email.prototype.onConfirmation = function onConfirmation(blk, tx, conf, app) {
  ...
  if (conf == 0) {
    app.browser.notification("New Email",tx.transaction.msg.title);
...
Email.prototype.attachEmailEvents = function attachEmailEvents() {
  ...
  // ADDED TO MAILCHAT
  $('.lightbox_message_from_address').off();
  $('.lightbox_message_from_address').on('click', function() {
    let x = confirm("Add "+$(this).html()+" to chat?");
    if (x == 1) {
      $('.sidechat').show();
      $('.sidechat_controls').show();
      let cmd = "add "+$(this).html();
  ...
  $('.lightbox_message_to_address').off();
  $('.lightbox_message_to_address').on('click', function() {
    let x = confirm("Add "+$(this).html()+" to chat?");
    if (x == 1) {
      $('.sidechat').show();
      $('.sidechat_controls').show();
