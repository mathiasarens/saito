...
Encrypt.prototype.displayEmailMessage = function displayEmailMessage(app, message_id) {

  if (app.BROWSER == 1) {
    this.email_view_txsig = message_id.substring(8);
    ...
    new_auth_link = "encrypt_authorize_link_"+this.email_view_txsig;
...
Encrypt.prototype.onConfirmation = function onConfirmation(blk, tx, conf, app) {
  ...
  if (conf == 0) {
    ...
    //
    // do we already have a key generated with these ppl
    //
    ...
    if (sender == app.wallet.returnPublicKey()) {
      if (request == "key exchange request") {
          ...
          msg.data            = "You have requested a key exchange request with:<p></p>"+receiver+"<p></p>You will be notified when they approve this request.";
          msg.alice_publickey = txmsg.alice_publickey;
          msg.markdown = 0;
	...
	if (Big(tx.transaction.to[0].amt).gt(0)) {
          ...
          msg.time            = tx.transaction.ts;
          msg.module          = "Email";
          msg.title           = "Approved Encryption Request";
          msg.data            = "Your account has created a private key for communicating securely on-chain with "+tx.transaction.from[0].add;
          msg.markdown = 0;
	...
	var bob_publickey = new Buffer(txmsg.bob, "hex");;
	...
	var alice            = app.crypto.createDiffieHellman(alice_publickey, alice_privatekey);
        var alice_secret     = app.crypto.createDiffieHellmanSecret(alice, bob_publickey);
...
Encrypt.prototype.approveEncryptionRequest = function approveEncryptionRequest(app, thistx, fee) {
  ...
  alice_publickey = thistxmsg.alice_publickey;

  // generate shared secret and save
  bob              = app.crypto.createDiffieHellman();
  bob_publickey    = bob.getPublicKey(null, "compressed").toString("hex");
  bob_privatekey   = bob.getPrivateKey(null, "compressed").toString("hex");
  bob_secret       = app.crypto.createDiffieHellmanSecret(bob, new Buffer(alice_publickey, "hex"));
  ...
  app.mempool.addTransaction(newtx, 1);  // 1 = relay on validate
...
Encrypt.prototype.attachEmailEvents = function attachEmailEvents(app) {
  ...
  if (app.BROWSER == 1) {
    ...
    $('.encrypt_authorize_link').on('click', function() {
      ...
      let thistx = app.archives.returnTransactionBySig(txid);

      if (thistx == null) { alert("Cannot Find TX by Id: "+txid); return; }
      if (app.keys.hasSharedSecret(thistx.transaction.from[0].add)) { 
        app.modules.returnModule("Email").showBrowserAlert("a shared secret now exists between your accounts!");
