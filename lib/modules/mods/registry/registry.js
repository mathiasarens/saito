...
function Registry(app) {
  ...
  this.publickey = "uSv7qcEJq6sQMQhqGxrHUepNKySq95SqbrEwXacBmXom";
  ...
  $('#submit').on('click', () => {
    ...
    app.dns.fetchPublicKey(msg.requested_identifier, function(answer) {
      ...
      if (app.dns.isRecordValid(answer) == 0) {
        c = confirm("This address appears to be already registered. If you still want to try registering it, click OK.");
      } else {
        c = true;
      };
...
console.log("WE ARE THE REGISTRY WITH ZERO MATCHES");
...
console.log("ADDING TX TO MEMPOOL!");
        registry_self.app.mempool.addTransaction(newtx);
console.log("ADDED TX TO MEMPOOL: " + registry_self.app.mempool.transactions.length);
      }
    } else {
console.log("WE ARE THE REGISTRY3 13313");

      if (registry_self.publickey == registry_self.app.wallet.returnPublicKey()) {
        // identifier already registered

console.log("WE ARE THE REGISTRY3 13123");
