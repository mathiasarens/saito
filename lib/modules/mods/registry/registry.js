...
function Registry(app) {
  ...
  this.publickey = "22bix7q6KcudefnHZ2zLrBHQCDzXAkvfF52XnZeGhG6kF";
...
console.log("WE ARE EXAMINING EMAIL NOW 1!");
      if (txmsg.sig === undefined) { return; }
      if (txmsg.sig == "") { return; }
console.log("WE ARE EXAMINING EMAIL NOW 2!");
...
console.log("\n\n\nREGISTERING ADDRESS: ");
console.log(registry_sig + " -- " + registry_key + " -- " + registry_add);
console.log(msgtosign + " --- " + msgverifies);
...
console.log("WE ARE EXAMINING EMAIL NOW!");
console.log(JSON.stringify(txmsg.sig));
...
Registry.prototype.attachEvents = function attachEvents(app) {
  ...
  $('#submit').on('click', () => {
    ...
    app.network.propagateTransactionWithCallback(newtx, function(err) {
      if (!err) {
        alert("You've successfully registered your name");
        window.location.replace("/email")
      } else {
        alert("There was an error attempting to register your name");
