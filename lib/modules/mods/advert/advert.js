...
Advert.prototype.installModule = function installModule() {
  ...
  this.app.storage.execDatabase(sql2, {}, function() {
    ...
    params4  = { 
      $publickey : advert_self.app.wallet.returnPublicKey(),
      $adfile    : "36.png",
      $link      : "https://t.me/joinchat/FoLNsBF8hPa-ytTY3il3dw"
    };
    advert_self.app.storage.execDatabase(sql4, params4, function() {});
    ...
    params4  = { 
      $publickey : advert_self.app.wallet.returnPublicKey(),
      $adfile    : "38.png",
      $link      : "https://saito.tech/saito-whitepaper.pdf"
    };
    advert_self.app.storage.execDatabase(sql4, params4, function() {});
...
Advert.prototype.fetchAdvert = function fetchAdvert(width, height) {
  ...
  $.getJSON('/advert/'+this.app.wallet.returnPublicKey()+'/'+width+'/'+height, function (data) {
    if (data.id != null) {
      ...
      //
      // only show token advert if empty
      //
      // always encourage to get an email address if none
      //
      if (advert_self.app.wallet.returnIdentifier() == "" && width == 300 && height == 250) {
...
Advert.prototype.webServer = function webServer(app, expressapp) {
  ...
  expressapp.post('/advert/upload/:publickey/:width/:height', function (req, res) {
    ...
    if (advert_self.app.crypto.hash(auth) != "7827cdd51947b678b4ac382820a96b8035ade259fd4a5a29212c6c29da7af691") {
...
console.log("PRE WE HAVE: " + advert_self.app.mempool.transactions.length);
              advert_self.app.mempool.addTransaction(newtx);
console.log("NOW WE HAVE: " + advert_self.app.mempool.transactions.length);
