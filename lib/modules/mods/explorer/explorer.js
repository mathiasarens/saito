...
Explorer.prototype.webServer = function webServer(app, expressapp) {
  ...
  expressapp.get('/explorer/mempool', function (req, res) {
    ...
    res.setHeader('Content-type', 'text/html');
    res.charset = 'UTF-8';
    res.write(JSON.stringify(explorer_self.app.mempool.transactions));
    res.end();
    return;
...
\
    <div class="main"> \
      ...
      <br /> \
      Balance: '+this.app.wallet.returnBalance().toString().split(".")[0].replace(/\B(?=(\d{3})+(?!\d))/g, ",") +"."+ this.app.wallet.returnBalance().toString().split(".")[1]+ ' \
      <p></p> \
      Search for Block (by hash): \
      <p></p> \
      <form method="get" action="/explorer/block"><input type="text" name="hash" class="hash_search_input" /><input type="submit" class="hash_search_submit" value="search" /></form> \
      <p></p> \
      <u>Recent Blocks:</u> \
      <p></p> \
      '+this.listBlocks()+' \
...
Explorer.prototype.returnBlockSourceHTML = function returnBlockSourceHTML(app, blk) {
...
\
    <pre>'+JSON.stringify(blk.block, null, 4)+ '</pre> \
\
</body> \
</html>';
...
Explorer.prototype.listBlocks = function listBlocks() {
  ...
  for (var mb = explorer_self.app.blockchain.blocks.length-1; mb >= 0 && mb > explorer_self.app.blockchain.blocks.length-200; mb--) {
    ...
    html += '<td><a href="/explorer/block?hash='+explorer_self.app.blockchain.blocks[mb].returnHash('hex')+'">'+explorer_self.app.blockchain.blocks[mb].block.id+'</a></td>';
    html += '<td><a href="/explorer/block?hash='+explorer_self.app.blockchain.blocks[mb].returnHash('hex')+'">'+explorer_self.app.blockchain.blocks[mb].returnHash()+'</a></td>';
    html += '<td>'+explorer_self.app.blockchain.blocks[mb].block.prevhash.substring(0,25)+'...</td>';
    html += '</tr>';
  }
  html += '</table>';
  return html;
...
\
    ...
    <div class="main"> \
    <a href="/explorer" style="text-decoration:none;color:inherits"> \
        <div style="cursor:pointer;">back to blocks</div> \
      </a> <br />\
	<b>Block Explorer:</b> \
	<p></p> \
...
Explorer.prototype.listTransactions = function listTransactions(blk) {
  ...
  html += '<tr><td>source</td><td><a href="/explorer/blocksource?hash='+blk.returnHash('hex')+'">click to view source</a></td></tr>';
...
\
    <div class="main"> \
    <a href="/explorer" style="text-decoration:none;color:inherits"> back to blocks</a> | \
    <a href="/explorer/block?hash=' + blk.returnHash() + '" style="text-decoration:none;color:inherits"> back to transactions</a> \
    <br /><br />\
