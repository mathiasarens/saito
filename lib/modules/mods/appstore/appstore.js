...
function AppStore(app) {
  ...
  this.dir                  = path.join(__dirname, "../../../data/appstore.sq3");
  this.db                   = null;
...
// Install Module //
////////////////////
AppStore.prototype.installModule = async function installModule(app) {
  ...
  try {
    this.db = await sqlite.open('./data/appstore.sq3');
...
AppStore.prototype.initialize = async function initialize(app) {
...
AppStore.prototype.dbRun = async function dbRun(sql, params, mycallback) {
  try {
...
AppStore.prototype.dbAll = async function dbAll(sql, params, mycallback) {
  try {
...
AppStore.prototype.dbGet = async function dbGet(sql, params, mycallback) {
  try {
...
AppStore.prototype.webServer = function webServer(app, expressapp) {
  ...
  expressapp.get('/appstore', function (req, res) {
    ...
    appstore_self.dbAll(psql, pparams, function(err, rows) {
      if (err != null) {
	      console.log(err);
	      return;
      }
...
console.log("assigning values");
  ...
  expressapp.post('/appstore/generateExtension', function (req, res) {
    ...
    appstore_self.dbAll(msql, mparams, function(err, rows) {
      if (err) {
        appstore_self.currently_generating = 0;
        return;
      };
...
AppStore.prototype.finishCompilationProcess = function finishCompilationProcess() {
  ...
  if (shell.exec(mkdircmd).code !== 0) {
    ...
    // generating the initial browser.hs -- handled by compile
    // and copy it into our chrome extension
    //let createcmp5 = "sed 's/app.BROWSER    = 0;/app.BROWSER    = 1;/g' " + __dirname + "/saitolib/start.js > " + __dirname + "/web/cache/"+appstore_self.unique_no+"/browser.js";
    //console.log("\n\n\n\n\n!!!\n\n\n\n\nTHIS: " + createcmp5);

    // old bad
    let createcmp5 = "cp -f " + __dirname + "/saitolib/saito/web/browser.js " + __dirname + "/web/cache/"+appstore_self.unique_no+"/browser.js";
console.log(createcmp5);
...
console.log("GENERATED CHROME PLUGIN");
                  ...
                  let email_module_indexfile = __dirname + "/saitolib/modules/mods/email/web/index.html";
		  if (!fs.existsSync(email_module_indexfile)) {
		    console.log("Attempt to create a chrome extension without email!");
		    return;
	          }
...
console.log(copyemaildir);
          	  var shell_success;
          	  try {
          	    shell_success = shell.exec(copyemaildir).code
          	  } catch (err) {
          	    console.log(err);
          	  }
          	  if ( shell_success !== 0) {
...
AppStore.prototype.displayEmailForm = function displayEmailForm(app) {
  ...
  element_to_edit_html += '<div style="font-size:0.8em">This module can be used to upload Saito applications to the blockchain.</div>';
  element_to_edit_html += '<p></p>';
  ...
  $('.app_id').val(appstore_self.app.crypto.hash(Math.random().toString().substring(2, 15)));
...
AppStore.prototype.onConfirmation = function onConfirmation(blk, tx, conf, app) {
  ...
  if (conf == 0) {
	...
	$version     : version,
      ...
      appstore_self.dbRun(sql, params, function(err) {});
    }
  }
}
