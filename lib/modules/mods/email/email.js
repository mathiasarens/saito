...
Email.prototype.installModule = function installModule(app) {
  ...
  <p></p>
  1. Visit our <a style="text-decoration:underline;color:#444;text-decoration:underline;" href="/faucet?mobile=no&saito_app=email&saito_address=${app.wallet.returnPublicKey()}">token faucet</a>.
  <p></p>
...
Email.prototype.attachMessage = function attachMessage(message, app) {
  ...
  if (msg.to == undefined) { msg.to = app.wallet.returnPublicKey(); }
     ...
     <td>
       <table class="inner_message_table">
         <tr class="inner_message" id="message_${message.id}">
           ...
           <td id="message_${message.id}_to" class="to" style="display:none">${this.formatAuthor(message.to, app)}</td>
...
Email.prototype.updateBlockchainSync = function updateBlockchainSync(app, percent_downloaded) {
  if (app.BROWSER == 0) { return; }
...
Email.prototype.attachEvents = function attachEvents(app) {
  ...
  $('#mailbox_inbox').on('click', function() {
    ...
    app.archives.processTransactions(40, function (err, txarray) {
      for (var bv = 0; bv < txarray.length; bv++) {
        try {
          if (txarray[bv].transaction.msg.module == "Email" || txarray[bv].transaction.msg.module == "Encrypt") {
            if (txarray[bv].transaction.to[0].add == email_self.app.wallet.returnPublicKey()) {
              email_self.addMessageToInbox(txarray[bv], app);
            }
          }
        } catch (err) {
  ...
  $('#reply').on('click', function() {
    ...
    let tt = $('.mail-module-label').html();
    email_self.closeMessage();
    $('#compose').click();
    $('.lightbox_compose_from_address').val(to);
    $('.lightbox_compose_to_address').val(fr);
    $('.email_title').val("Re: [" + tt + "]");

    if (email_self.app.keys.hasSharedSecret(fr)) {
  ...
  $('#lightbox_compose_to_address').on('focusout', function() {
    ...
    if (email_self.isPublicKey(recipient) == 0) {
      ...
      recipientkeys = email_self.app.keys.findByIdentifier(recipient);
        ...
        if (email_self.app.keys.hasSharedSecret(recipientkeys.publickey)) {
          $('#lightbox_compose_to_address').css('background-color', '#f4fa58');
          $('#lightbox_compose_from_address').css('background-color', '#f4fa58');
      ...
      // highlight if can encrypt
      if (email_self.app.keys.hasSharedSecret(recipient)) {
        $('#lightbox_compose_to_address').css('background-color', '#f4fa58');
        $('#lightbox_compose_from_address').css('background-color', '#f4fa58');
        //email_self.showEmailAlert("<span class='yellow-bg'>[encrypted email]</span>");
      } else {
  ...
  $('#compose').on('click', function() {
    ...
    if (email_self.isPublicKey(to) == 0) {
      ...
      $('#lightbox_compose_from_address').css('color', '#0B6121');
      // highlight if can encrypt
      if (email_self.app.keys.hasSharedSecret(to)) {
        $('#lightbox_compose_to_address').css('background-color', '#f4fa58');
        $('#lightbox_compose_from_address').css('background-color', '#f4fa58');
        //email_self.showEmailAlert("<span class='yellow-bg'>[encrypted email]</span>");
      }
    ...
    $('#send').on('click', function() {
      ...
      if (to.indexOf("@saito") > 0) {
        ...
        if (pk == "") {
          ...
          if (newtx == null) {
            console.log("NULL");
            email_self.showBrowserAlert("Error, refresh the browser and try again");
            return;
          }
          ...
          app.network.propagateTransactionWithCallback(newtx, function(err) {
            console.log("TESTING SEND EMAil6!");
            if (err) {
              console.log(err);
              email_self.showBrowserAlert(err);
            } else {
              email_self.showBrowserAlert("your message has been broadcast to the network");
              email_self.closeMessage();
            }
  ...
  $('.inner_message').on('click', function() {
    ...
    email_self.app.modules.displayEmailMessage(module, message_id);
    ...
    <div class="lightbox_compose_module" id="lightbox_compose_module">
      <div class="lightbox_compose_module_box">
        <div class="lightbox_compose_fee_container" style="float:left;font-size:0.9em;">
          ...
          <input type="text" class="lightbox_compose_fee" id="lightbox_compose_fee" value="${this.app.wallet.returnDefaultFee()}" />
        </div>
...
// Display Message //
/////////////////////
Email.prototype.displayEmailMessage = function displayEmailMessage(app, message_id) {
...
Email.prototype.formatEmailTransaction = function formatEmailTransaction(tx, app) {
  ...
  // encrypt if email msg
  if (this.active_module == "Email") {
...
Email.prototype.attachEmailEvents = function attachEmailEvents() {
  ...
  if (this.app.BROWSER == 1) {
    ...
    $('.register_email_address').on('click', function() {
      ...
      $('#primary_nav_wrap ul ul').css('display', 'none');
      $('.mail-module-label').html("Registry");
      $('.mail-module-label').show();
      $('.mail-controls-btn').hide();
      $('.mail-controls-back').show();
    });
...
//
Email.prototype.sendMail = function sendMail(to, title, body, amount, fee, module, callback) {

  let newtx = app.wallet.createUnsignedTransaction(this.app.wallet.returnPublicKey(), amount, fee);
  if (newtx == null) { return; }

  newtx = app.modules.formatEmailTransaction(newtx, module.name);
  newtx = app.wallet.signTransaction(newtx);
  this.showEmailAlert("<span id='sendingthismsg'>attempting to broadcast transaction....</span>");
  if (callback == null) {
    app.network.propagateTransactionWithCallback(newtx, function() {
      email_self.showBrowserAlert("your message has been broadcast to the network");
      email_self.closeMessage();
    ...
    app.network.propagateTransaction(newtx, 1);
    callback();
  }
