...
Facebook.prototype.initialize = function initialize() {
  ...
  if (this.app.BROWSER == 1) {
    ...
    //
    // check the server for recent posts
    //
...
Facebook.prototype.initializeHTML = function initializeHTML(app) {
  ...
  if (app.wallet.returnIdentifier() != "") {
    ...
    $('#saitoname').text(this.formatAuthor(app.wallet.returnPublicKey(), this.app));
  }
...
console.log("HERE WE ARE BUG-HUNTING: ");
...
alert("updating friend list");
  // update friend list
  let facebook_friends = this.app.keys.returnKeychainByTag("Facebook");
alert(facebook_friends.length + " -- " + JSON.stringify(facebook_friends));
  if (facebook_friends.length > 0) {
    $('#following').show();
    $("#following_friends_box").html('');
    $('#who_follow').css('margin-top', '10px');
    for (let ff = 0; ff < facebook_friends.length; ff++) {
...
alert("hiding following 3!");
    $("#following_friends_box").html("<h2>You have no friendZZZ</h2>");
    $('#following').hide();
alert("hidden -- following 3!");
  }
  ...
  setTimeout(function() {
    ...
    $('.comment_name').each(function() {
      ...
      if (app.crypto.isPublicKey(commentname) == 1) {
        updateme = 1;
...
Facebook.prototype.addPostToWall = function addPostToWall(tx, app, prepend = 0) {
  ...
  if (app.BROWSER == 0) { return; }
    ...
    msg.type   = tx.transaction.msg.type;
    msg.data   = tx.transaction.msg.data;
    msg.markdown = tx.transaction.msg.markdown;
  ...
  return `
    <div class="post_box" id="post_box_create">
      ...
      <div class="post_create">
        <textarea id="post_create_textarea" class="post_create_textarea"></textarea>
      </div>
...
Facebook.prototype.formatPost = function formatPost(msg, app) {
  var html = `
   ...
   `;
   if (msg.markdown == 0) {
      html += `<div class="post_content">${msg.data}</div>`;
   } else {
      html += `<div class="post_content">${markdown.toHTML(msg.data)}</div>`;
   }
   html += `
...
Facebook.prototype.attachEvents = function attachEvents(app) {
  ...
  $('#add_friend').on('click', function() {
    ...
    $.fancybox({
	  ...
	  if (app.crypto.isPublicKey(newfriend) == 1) {
...
';
  ...
  $('#publish_button').on('click', function() {
    ...
    facebook_self.creating_new_post = 0;
  ...
  $('#new_post').on('click', function() {
    if (facebook_self.creating_new_post == 1) { alert("You can only create one post at a time."); return; }
    if (facebook_self.app.wallet.returnBalance() == 0) {
      let getTokens = confirm("You need Saito tokens to post. Click OK to visit our faucet");
      if (getTokens == 1) { location.href = "/faucet?saito_app=facebook"; }
      return;
    }
    facebook_self.creating_new_post = 1;
  ...
  $('.post_controls_like').on('click', function() {
    ...
    if (facebook_self.app.wallet.returnBalance() == 0) {
       let getTokens = confirm("You need Saito tokens to like a post. Click OK to visit our faucet");
       if (getTokens == 1) { location.href = "/faucet?saito_app=facebook"; }
       return;
    }
  ...
  $('.post_controls_comment').on('click', function() {
     ...
     if (facebook_self.app.wallet.returnBalance() == 0) {
       let getTokens = confirm("You need Saito tokens to comment. Click OK to visit our faucet");
       if (getTokens == 1) { location.href = "/faucet?saito_app=facebook"; }
       return;
     }
     id = $(this).attr('id');
  ...
  $('.post_comments_button').on('click', function() {
    ...
    if (tmpcomment === "") {
      alert("Comment is empty, not posting.");
      return;
    }
  ...
  $('.post_controls_delete').on('click', function() {
    ...
    $(tmpselector).remove();
    facebook_self.app.archives.removeMessage(facebook_self.app.archives.returnTransactionById(msgid).transaction.sig);
  });
  ...
  $('.post_controls_share').on('click', function() {
    ...
    if (facebook_self.app.wallet.returnBalance() == 0) {
       let getTokens = confirm("You need Saito tokens to share a post. Click OK to visit our faucet");
       if (getTokens == 1) { location.href = "/faucet?saito_app=facebook"; }
       return;
    }
    ...
    tmpselector  = "#post_box_" + msgid + " > .post_header > .post_header_titlebox > .post_header_address";
    postauthorpk = $(tmpselector).text();

    tmpselector  = "#post_box_" + msgid + " > .post_header > .post_header_titlebox > .post_header_name";
    postauthorid = $(tmpselector).text();

    tmpselector  = "#post_box_" + msgid + " > .post_content";
    postbody     = $(tmpselector).html();

console.log("\n\nWE HAVE FOUND: " + postauthorpk + " -- " + postauthorid + " -- " + postbody);
    ...
    share_email = 'Your post has been shared by:<p></p>'+ myid;
    ...
    newtx.transaction.msg.title   = "Your post has been shared!";
    newtx.transaction.msg.markdown = 0;
    newtx = app.wallet.signTransaction(newtx);
    app.network.propagateTransaction(newtx);
    app.storage.saveOptions();
...
Facebook.prototype.onConfirmation = function onConfirmation(blk, tx, conf, app) {
  ...
  if (conf == 0) {
    ...
    if (app.keys.isWatched(tx.transaction.to[0].add)) {
      am_i_following = 1;
    }
...
Facebook.prototype.formatAuthor = function formatAuthor(author, app, msg=null) {
  ...
  if (this.app.crypto.isPublicKey(author) == 1) { 
    if (this.app.dns.isActive() == false) { return author; }
    if (msg != null) {
...
Facebook.prototype.generateTransactionByPostId = function generateTransactionByPostId(app, post_id, childtx=null) {
  ...
  // we sign ourselves to generate unique mhash
  newtx = app.wallet.signTransaction(newtx);
