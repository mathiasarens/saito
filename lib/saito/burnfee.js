...
function BurnFee() {
  ...
  // default values
  this.start         = 10;
  this.heartbeat     = 10;        // expect new block every 30 seconds
...
BurnFee.prototype.burnFeeAdjustment = function burnFeeAdjustment(prevblk=null, blk=null) {
  ...
  console.log("CALCULATING THE ADJUSTED BURN FEE");
  console.log("BF START: ", prevblk.block.bf.start);
  console.log("THIS.HEARTBEAT: ", this.heartbeat);
  console.log("BLOCK TIMESTAMP: ", blk.block.ts);
  console.log("PREVBLOCK TIMESTAMP: ", prevblk.block.ts);
...
BurnFee.prototype.adjustBurnFee = function adjustBurnFee(prevblk=null, blk=null) {
  ...
  if (prevblk == null || blk == null) {
    console.log("BURN FEE IS RETURNED DEFAULT: ", this.start);
    return { start: this.start, current: 0 };
  }

  var bf = {};
  bf.start = this.burnFeeAdjustment(prevblk, blk);
  console.log("BURN FEE IS ADJUSTED: ", bf.start);
  bf.current = 0;
  if (prevblk != null && blk != null) {
