          ...
          //
            ...
            $.get(
              block_to_download_url, {
	            ...
	            console.log("could not add block in fetch block for clients, so did not run processBlocks");
	          }
                } else {
...
Mempool.prototype.removeBlockAndTransactions = function removeBlockAndTransactions(blk=null) {
  ...
  console.log("TX IN MEMPOOL", this.transaction.length);
  console.log("TX INPUTS IN HMAP", this.transactions_inputs_hmap);
}
...
Mempool.prototype.purgeExpiredGoldenTickets = function purgeExpiredGoldenTickets(prevblk_hash="") {
  console.log("PURGING EXPIRED GOLDEN TICKETS");
  for (let i = this.transactions.length - 1; i >= 0; i--) {
    if (this.transactions[i].transaction.type == 1) {
