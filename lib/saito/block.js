...
function Block(app, blkjson = "", confirmations = -1) {
  ...
  this.block.treasury = Big("2868100000.0");  // 51% locked into floating coinbase 
  // ~20% development and sale
  // remainder network allocation
  ...
  if (blkjson != "") {
    try {
      //
      // we must force into utf-8 to prevent encoding issues
      //
  ...
  if (this.transactions.length > 0) {
    ...
    ////////////////////////////////////////
    // recreate the proper txsjson format //
    ////////////////////////////////////////
    for (let i = 0; i < this.transactions.length; i++) {
      this.block.txsjson[i] = this.transactions[i].stringify(1);  // 1 = escape dbl quotes
    }
...
Block.prototype.bundle = async function bundle(prevblk = null) {
  ...
  // update burn fee
  //
  console.log("PREVIOUS BLOCK: ", prevblk);
  this.block.bf = this.app.burnfee.adjustBurnFee(prevblk, this);
  console.log("BLOCK BURN FEE", this.block.bf);
...
console.log("ELIMINATED BLOCK ID: " + eliminated_block_id + " -- " + goldenticket_block_id);

  if (eliminated_block_id < 1) { return []; }

console.log(" ... loading eid: " + eliminated_block_id);
...
console.log(" ... checking spent? " + JSON.stringify(slip));
...
console.log(" ... slip is rebroadcast friendly!");
...
Block.prototype.validate = async function validate() {
  ...
  try {
    ...
    console.log("\n\n\nTOTAL IN CUMULATIVE FEES: " + block_cumulative_fees + "\n\n");
...
console.log("\n\nCALCULATING RECLAIMED FUNDS: ");
console.log(JSON.stringify(block_reclaimed));
console.log("\n\n");
    ...
    if (gttx == null) {
      ...
      console.log("THE WINNING NODE IS: " + winning_node);
      ...
      console.log("AND THE PAYOUT IS TO: " + gttx.transaction.to[1].add);
 ...
 */
Block.prototype.stringify = function stringify() {
  ...
  let json = '{'
    ...
    + '"id":' + this.block.id + ','
    + '"bf":{"start": ' + this.block.bf.start + ',"current":' + this.block.bf.current + '},'
    + '"difficulty":' + this.block.difficulty + ','
    + '"paysplit":' + this.block.paysplit + ','
    + '"treasury":"' + this.block.treasury + '",'
    + '"coinbase":"' + this.block.coinbase + '",'
    + '"reclaimed":"' + this.block.reclaimed + '",'
    + '"vote":' + this.block.vote + ','
    + '"txsjson": ['
    + "\n";
  //
  // if we already have the string, re-use it
  //
  for (let i = 0; i < this.block.txsjson.length; i++) {
    if (i != 0) { json += ",\n"; }
    json += '"' + this.block.txsjson[i] + '"';
  }
